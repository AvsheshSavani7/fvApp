...
const StaircaseCheckListPdf = ({ customerDetails1 }) => {
  ...
  const SingleCheckList = ({ checklist, index }) => {
    ...
    const findnameOfStaircase = allQuestions?.find(
      (Q) => Q?.question === "Name of Staircase"
    );
    ...
    const findQues = onlyQwithAnswer.find(
      (Q) => Q.question === "Exposed/Covered"
    );
    const groupedArray = [];
    if (findQues?.answer === "Exposed") {
      ...
      let addedQuesArray = [
        ...onlyQwithAnswer.filter((Q) => Q?.question !== "Exposed/Covered"),
        findQues,
      ];
      let subQuestion = findQues?.subQuestion?.filter((Q) => {
        return Q.whenToShow === findQues?.answer && Q?.answer !== "none";
      });
      if (subQuestion?.length > 0) {
        addedQuesArray.push(subQuestion[0]);
      }
      for (let i = 0; i < addedQuesArray.length; i += 2) {
        const pair = [addedQuesArray[i]];
        if (i + 1 < addedQuesArray.length) {
          pair.push(addedQuesArray[i + 1]);
        }
        groupedArray.push(pair);
      }
    }
    ...
    const findRoomName = () => {
      customerDetails1?.fv_data?.scope?.floors?.map((floor) => {
        if (floor?.id === checklist?.from_floor_id) {
          fromRoomName = floor?.name;
        } else if (floor?.id === checklist?.to_floor_id) {
    ...
    return (
      <>
        ...
        >
          ...
          >
            Staircase - {findnameOfStaircase?.answer || "Unknown"}
          </Text>
...
export default StaircaseCheckListPdf;
